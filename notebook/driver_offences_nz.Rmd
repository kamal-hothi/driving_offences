...
alc_vs_mob <- c("Alcohol specific offences", "Mobile phone offences")

offences_and_fines %>% filter(type_of_offence == alc_vs_mob) %>% 
  filter(count_category=='Offences') %>%
...
offences_and_fines_by_year <- offences_and_fines %>%   mutate(year = year(month_year)) %>% filter(type_of_offence == alc_vs_mob) %>%
group_by(year, type_of_offence, count_category) %>%
summarise(count =  sum(count)) %>% ungroup() %>% spread(type_of_offence, count)

offences_and_fines_by_year
